...
class MLB_Scrape:
    ...
    def get_player_pitches(self,
                            ...
                            sport_id: int = 1,
                            game_type: list = ['R'],
                            pitching: bool = True) -> pl.DataFrame:
        """
        Retrieves all individual pitch-level data for a specific player in a season,
        ...
        - sport_id (int): Sport ID (default 1 = MLB).
        - game_type (list): Game types to include, e.g. ['R'] for regular season.
        - pitching (bool): If True, filters to pitches this player threw.
                            If False, filters to pitches this player faced (as batter).

        Returns:
        - player_df (pl.DataFrame): Pitch-level data for just this player, within the date range.
        """
        # Step 1: find every game the player appeared in (already narrowed by date range)
        game_list = self.get_player_games_list(
            player_id=player_id,
            season=season,
        ...
        # Step 2: pull live feed data for those games
        data = self.get_data(game_list)

        # Step 3: flatten into a full pitch-level dataframe
        full_df = self.get_data_df(data)

        # Step 4: filter down to just this player's pitches
        id_col = 'pitcher_id' if pitching else 'batter_id'
        player_df = full_df.filter(pl.col(id_col) == player_id)

        # Step 5: enforce the date range directly on pitch data too, as a safety net
        # (game_date is a string like 'YYYY-MM-DD'; cast to pl.Date for correct comparison)
        if start_date or end_date:
            player_df = player_df.with_columns(
                pl.col('game_date').str.to_date()
            )
            if start_date:
                player_df = player_df.filter(pl.col('game_date') >= pl.lit(start_date).str.to_date())
            if end_date:
                player_df = player_df.filter(pl.col('game_date') <= pl.lit(end_date).str.to_date())
